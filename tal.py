...
##### MODIFIED FROM "ultralytics.utils.tal.TaskAlignedAssigner"
class TaskAlignedAssigner(nn.Module):
    ...
    ##### BEGIN: MODIFIED
    @torch.no_grad()
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt, gt_labels_sub = None):
        """
        ...
        try:
            ##### BEGIN: MODIFIED
            return self._forward(pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt, gt_labels_sub)
            ##### END: MODIFIED
            ...
            ##### BEGIN: MODIFIED
            cpu_tensors = [t.cpu() for t in (pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt, gt_labels_sub)]
            ##### END: MODIFIED
    ...
    ##### BEGIN: MDIFIED
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt, gt_labels_sub = None):
        """
        Compute the task-aligned assignment.
        ...
        # Assigned target
        ##### BEGIN: MODIFIED
        target_labels, target_bboxes, target_scores = self.get_targets(gt_labels, gt_bboxes, target_gt_idx, fg_mask, gt_labels_sub)
        ##### END: MODIFIED
    ...
    ##### BEGIN: MODIFIED
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask, gt_labels_sub = None):
        """
        ...
        ##### BEGIN: MODIFIED
        target_labels_sub = [
            gt_labels_sub[:, :, idx].long().flatten()[target_gt_idx] for idx in range(gt_labels_sub.shape[-1])
        ]
        ##### END: MODIFIED
        ...
        ##### BEGIN: MODIFIED
        for idx in range(len(target_labels_sub)):
            target_labels_sub[idx].clamp_(0)
        ##### END: MODIFIED
        ...
        ##### BEGIN: MODIFIED
        for idx in range(len(target_labels_sub)):
            target_scores.scatter_(2, target_labels_sub[idx].unsqueeze(-1), 1)
        ##### END: MODIFIED
