...
##### MODIFIED FROM "ultralytics.data.base.BaseDataset"
class BaseDataset(Dataset):
    ...
    def update_labels(self, include_class: Optional[list]):
        ...
        for i in range(len(self.labels)):
            if include_class is not None:
                ...
                ##### BEGIN: MODIFIED
                self.labels[i]['cls_sub'] = cls_sub[j]
                ##### END: MODIFIED
    ...
    def __getitem__(self, index):
        ...
        # print(f'[0] -> {sorted(_ret.keys())}')
        # print(f'\t[CLS] --> {type(_ret["cls"])}, {_ret["cls"].shape}, [CLS_SUB] --> {type(_ret["cls_sub"])}, {_ret["cls_sub"].shape}')

        _ret = self.transforms(_ret)

        # print(f'[1] -> {sorted(_ret.keys())}')
        # print(f'\t[CLS] --> {type(_ret["cls"])}, {_ret["cls"].shape}, [CLS_SUB] --> {type(_ret["cls_sub"])}, {_ret["cls_sub"].shape}')
